    ...
    <Box>
      <Box mt="20px" sx={{ display: 'flex', flexWrap:'wrap', gap:3}}>
        <Stack direction="column" width="100%">
          <Typography fontSize={25} fontWeight={700} color='#11142d'>
              {!allProperties.length ? "Il n'y a pas de propriétés" : "Touts les propriétés"} </Typography>
              <Box 
                marginBottom={2} 
                mt={3} display="flex" 
                width="84%"
                ...
                <Box 
                  ...
                  <CustomButton 
                    title={`Trier le prix ${currentPrice === 'asc' ? '↑' : '↓'}` }
                    handleClick={() =>  toggleSort('price')}  
                    backgroundColor="#275be8"
                    color="#fcfcfc"
                  />
                <TextField
                  variant='outlined'
      ...
      {allProperties.length > 0 && (
        ...
        alignItems="center" gap="5px">
          Page{" "}<strong>{current} / {pageCount}</strong>
        </Box>
          ...
          />
                ...
                >
                  {[10, 20, 30, 40, 50].map((size) => (
                    <MenuItem key={size} value={size}>Montrer {size}</MenuItem>
                  ))}
